...
client.ev.on('call', item => console.log('recv call event', item))
  
global.conn = client
conn.autoReconnect = 0
//conn.connectOptions.alwaysUseTakeover = true
//client.autoReconnect = ReconnectMode.onConnectionLost;
//client.connectOptions.maxRetries = 10000;
...
else if (reason === DisconnectReason.connectionClosed) { console.log(color("[SYSTEM]", "white"), color('Connection closed, reconnecting...', 'deeppink')); client.end(`Unknown DisconnectReason: ${reason}|${connection}`) }
else if (reason === DisconnectReason.connectionLost) { console.log(color("[SYSTEM]", "white"), color('Connection lost, trying to reconnect', 'deeppink')); client.end(`Unknown DisconnectReason: ${reason}|${connection}`); }
...
client.ev.on('close', async (reason, isReconnecting) => {
    
    console.log("Disconnected because " +reason.reason)
    
    if(reason.reason === 'lost'){
        conn.close()
        conn.loadAuthInfo(conn.base64EncodedAuthInfo ())
        conn.connect()
        return {}
    }
...
id,
name: 'WhatsApp'
} : areJidsSameUser(id, client.user.id) ?
client.user :
(store.contacts[id] || {})
return (withoutContact ? '' : v.name) || v.subject || v.verifiedName || v.notify || PhoneNumber('+' + jid.replace('@s.whatsapp.net', '')).getNumber('international').replace(new RegExp("[()+-/ +/]", "gi"), "") 
 }
    ...
    */
    ...
    let message = await prepareWAMessageMedia({ video: gam1, gifPlayback: true, gifAttribution: 1}, { upload: client.waUploadToServer })
...
return client.relayMessage(jid, tod.message, {messageId: tod.key.id})
    ...
    */
    client.translate = async (code, text) => {
